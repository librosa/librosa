...
def test_cqt_bad_hop(y_cqt, sr_cqt, hop_length, bpo):
    ...
    librosa.cqt(y=y_cqt, sr=sr_cqt, hop_length=hop_length, n_bins=bpo*6,
                bins_per_octave=bpo)
...
def test_cqt(y_cqt, sr_cqt, hop_length, fmin, n_bins, bins_per_octave,
             tuning, filter_scale, norm, res_type, sparsity):

    C = librosa.cqt(y=y_cqt, sr=sr_cqt, hop_length=hop_length,
                    fmin=fmin, n_bins=n_bins, bins_per_octave=bins_per_octave,
                    tuning=tuning, filter_scale=filter_scale,
                    norm=norm, sparsity=sparsity, res_type=res_type)
...
def test_hybrid_cqt(y_hybrid, sr, hop_length, fmin, n_bins, bins_per_octave,
    ...
    C2 = librosa.hybrid_cqt(y_hybrid, sr=sr,
                            hop_length=hop_length,
    ...
    C1 = np.abs(librosa.cqt(y_hybrid, sr=sr,
                            hop_length=hop_length,
                            fmin=fmin, n_bins=n_bins,
                            bins_per_octave=bins_per_octave,
                            tuning=tuning, filter_scale=resolution,
...
@pytest.mark.parametrize('y', [np.sin(2 * np.pi * librosa.midi_to_hz(60) *
                                      np.arange(2 * 22050) / 22050.)])
...
def test_hybrid_cqt_impulse(y_impulse, sr_impulse, hop_impulse):
    ...
    hcqt = librosa.hybrid_cqt(y=y_impulse, sr=sr_impulse,
                              hop_length=hop_impulse, tuning=0)
...
def test_cqt_white_noise(y_white, sr_white, fmin, n_bins, scale):
    ...
    C = np.abs(librosa.cqt(y=y_white, sr=sr_white,
                           fmin=fmin,
                           n_bins=n_bins,
                           scale=scale))

    if not scale:
        lengths = librosa.filters.constant_q_lengths(sr_white, fmin,
                                                     n_bins=n_bins)
...
def test_hybrid_cqt_white_noise(y_white, sr_white, fmin, n_bins, scale):
    C = librosa.hybrid_cqt(y=y_white, sr=sr_white,
                           fmin=fmin,
                           n_bins=n_bins,
                           scale=scale)
...
def test_icqt(y_icqt, sr_icqt, scale, hop_length, over_sample,
              length, res_type, dtype):

    bins_per_octave = over_sample * 12
    n_bins = 7 * bins_per_octave

    C = librosa.cqt(y_icqt, sr=sr_icqt, n_bins=n_bins,
                    bins_per_octave=bins_per_octave,
                    scale=scale,
                    hop_length=hop_length)
    ...
    if length:
        ...
        _len = None
    yinv = librosa.icqt(C, sr=sr_icqt,
                        scale=scale,
                        hop_length=hop_length,
                        bins_per_octave=bins_per_octave,
                        length=_len,

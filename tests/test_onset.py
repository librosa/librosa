...
def test_onset_strength_audio(ysr, feature, n_fft, hop_length, lag, max_size, detrend, center, aggregate):
    ...
    S = librosa.feature.melspectrogram(y=y, n_fft=n_fft,
                                       hop_length=hop_length)
...
@pytest.mark.parametrize('center', [False, True])
@pytest.mark.parametrize('aggregate', [None, np.mean, np.max])
def test_onset_strength_spectrogram(melspec_sr, feature, n_fft,
                                    hop_length, detrend, center, aggregate):
    S, sr = melspec_sr
...
def test_onset_detect_const(y, sr, hop_length):
    ...
    onsets = librosa.onset.onset_detect(y=y, sr=sr, onset_envelope=None,
                                        hop_length=hop_length)

    assert len(onsets) == 0


@pytest.mark.parametrize('units, ctx', [('frames', dnr()),
                                        ('time', dnr()),
                                        ('samples', dnr()),

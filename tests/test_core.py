...
def test_dtw():
    # This is an example taken from (page 140, Fig. 3.16):
    # Meinard Mueller
    ...
    mut_D, mut_wp = librosa.dtw(X, Y)

    assert np.allclose(gt_D, mut_D)


def test_dtw_alignment():
    # We construct two linear ramps, one double the size of the other in [0, 1].
    # DTW should be capable of aligning both sequences.
    ...
    # do the warping
    gt_Y = Y
    mut_Y = np.zeros_like(Y)
    for cur_warp in mut_wp:
        # we will make an error here but that's okay
        mut_Y[cur_warp[1]] = X[cur_warp[0]]

    assert np.allclose(gt_Y, mut_Y, atol=1e-1)


def test_dtw_subseq():
    # This test searches for a linear ramp embedded in noise sequences.

    # test sequences
    X = np.linspace(0, 1, 50)
    noise_seq = np.random.rand(100)
    Y = np.concatenate((noise_seq, noise_seq, X, noise_seq))

    # ground-truth
    gt_end_idx = 2*len(noise_seq) + len(X) - 1
    gt_start_idx = gt_end_idx - len(X) + 1

    mut_D, mut_wp = librosa.dtw(X, Y, subseq=True)

    # get start and end idx
    mut_end_idx = np.argmin(mut_D[-1, :])
    mut_start_idx = mut_wp[-1][1]

    assert gt_end_idx == mut_end_idx
    assert gt_start_idx == mut_start_idx

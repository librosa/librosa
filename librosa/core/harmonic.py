...
def salience(S, freqs, harmonic_vals, weights=None, aggregate=None,
             filter_peaks=True, kind='linear'):
    ...
    if weights is None:
        weights = np.ones((len(harmonic_vals), ))
    else:
        weights = np.array(weights, dtype=float)

    S_harm = harmonics(S, freqs, harmonic_vals, kind=kind)

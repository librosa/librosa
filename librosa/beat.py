...
def estimate_tempo(onset_envelope, sr=22050, hop_length=512, start_bpm=120,
    ...
    if start_bpm <= 0:
        raise LibrosaParameterError('start_bpm must be strictly positive')
...
def __beat_tracker(onset_envelope, bpm, fft_res, tightness, trim):
    ...
    if bpm <= 0:
        raise LibrosaParameterError('bpm must be strictly positive')
...
def __beat_track_dp(localscore, period, tightness):
    ...
    if tightness <= 0:
        raise LibrosaParameterError('tightness must be strictly positive')
